    ...
    def __call__(self, oriImg):
        ...
        for k in range(len(mapIdx)):
            ...
            if (nA != 0 and nB != 0):
                ...
                for i in range(nA):
                    for j in range(nB):
                        vec = np.subtract(can[j][:2], candA[i][:2])
                        norm = math.sqrt(vec[0] * vec[0] + vec[1] * vec[1])
                        norm = max(0.001, norm)
                        vec = np.divide(vec, norm)

                        startend = list(zip(np.linspace(candA[i][0], can[j][0], num=mid_num), \
                                            np.linspace(candA[i][1], can[j][1], num=mid_num)))
                        ...
                        if criterion1 and criterion2:
                            connection_candidate.append(
                                [i, j, score_with_dist_prior, score_with_dist_prior + candA[i][2] + can[j][2]])
                ...
                for c in range(len(connection_candidate)):
                    ...
                    if (i not in connection[:, 3] and j not in connection[:, 4]):
                        connection = np.vstack([connection, [candA[i][3], can[j][3], s, i, j]])
